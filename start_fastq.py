...
from copyfiles_fastq import copy_fastq
import os
import sys
...
Also, when you are not in the program you could use 'qstat -u username to check the qsub tasks' ''')
    ...
    def do_step(self,last_step):
        ...
        if (last_step==0):
            ...
            (download_name, success)=copy_fastq.func(dic['project_name'], origin,dest,sample_name,user=dic['user'])
            if (success==1):
                dic['status']='running'
        ...
        if (last_step==1):
            ...
            (download_name, success)=fastq2gz_helper.func(dic['project_name'],dic['download_name'], user=dic['user'])
            if (success==1):
    ...
    def step_name(self,step):
        if (step==0):
            return "Step 0: begin"
        if (step==1):
            return "Step 1: Copy fastq files to working directory"
        if (step==2):
            return "Step 2: Convert fastq files to fastqgz files"
        if (step==3):
